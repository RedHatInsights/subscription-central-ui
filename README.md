...
- The starter repo uses Travis to deploy the webpack build to another Github repo defined in `.travis.yml/custom_release.sh`
  - That Github repo has the following branches:
    - `ci-beta` (deployed by pushing to `master` or `main` on this repo)
    - `ci-stable` (deployed by pushing to `master-stable` on this repo)
    - `qa-beta` (deployed by pushing to `master` on this repo)
    - `qa-stable` (deployed by pushing to `master-stable` on this repo)
